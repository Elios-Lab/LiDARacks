...
def lidaracks(sph_point_clouds, severity_level=0, type_of_noise='EMI'):
    ...
    if type_of_noise == 'EMI':
        severity = [0.5, 1, 1.5, 2, 2.5][severity_level]   # amplitude [m]

        cycles_per_frame = 12
        rho_min, rho_max = 0.0, 5.0                        # valid sensor range [m]

        num_points = sph_point_clouds.shape[0]
        k = np.arange(num_points)
        phase_shift = np.random.uniform(0, 2 * np.pi)
        range_error = severity * np.sin(2 * np.pi * cycles_per_frame * k / num_points + phase_shift)

        sph_point_cloud_interfered = sph_point_clouds.copy()
        sph_point_cloud_interfered[:, 0] += range_error

        sph_point_cloud_interfered[:, 0] = np.clip(sph_point_cloud_interfered[:, 0], rho_min, rho_max)
